...
def calculate_score(cards) -> int:
    """Calculates the score of the hand and returns it [int]."""
    score = sum(cards)
...
def compare(c_score, p_score) -> tuple[str, bool]:
    if p_score == c_score:
        return "Neither"
    elif c_score == 0:
        return "Opponent"
...
def print_scores(c_score, p_score, final):
    if final:
...
def play(reset_game):
    ...
    game_active = check_score(computer_score, player_score)

    print_scores(computer_score, player_score, not game_active)

    users_turn = True

    while game_active and users_turn:
        ...
        if another_card == "y":
            ...
            game_active = check_score(computer_score, player_score)
            if game_active:
                print_scores(computer_score, player_score, False)
        else:
            users_turn = False
    ...
    print_scores(computer_score, player_score, True)        
    winner = compare(computer_score, player_score)
